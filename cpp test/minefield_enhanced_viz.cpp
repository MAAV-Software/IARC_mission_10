...
private:
    std::vector<Edge> voronoi_edges; //defines a vector of edges for the voronoi path
    
    void build_voronoi_diagram() {
        DT dt; //defines a DT graph, then we add the mines (CGALpoints) to it
        for (const auto& mine : mines) {
            dt.insert(CGALPoint(mine.x, mine.y));
        }
        
        voronoi_edges.clear(); //clears out old edges
        ...
        for (auto eit = dt.finite_edges_begin(); eit != dt.finite_edges_end(); ++eit) {
            ...
            //adds each edge within the boundries of the graph, making sure that if edges are too close to each other they are combined
    ...
    //the start of the voronoi search path, defines the graph for the search
    ...
    //runs dikstras search, finding the actual shortest path 
...
int main() {
    ...
    const int NUM_MINES = 100;  // Use reasonable number
